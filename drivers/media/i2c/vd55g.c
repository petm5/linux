...
struct vd55g_chip_info {
	...
	int patch_length;
	const u8 *patch_array;
	int patch_revision_major;
	int patch_revision_minor;
	u32 mipi_rate_min;
	u32 mipi_rate_max;
	u16 width;
	u16 height;
...
static const struct vd55g_chip_info vd55g0_chip_info = {
	...
	.patch_length = sizeof(vd55g0_cut1_patch_array),
	.patch_array = vd55g0_cut1_patch_array,
	.patch_revision_major = VD55G0_FWPATCH_REVISION_MAJOR,
	.patch_revision_minor = VD55G0_FWPATCH_REVISION_MINOR,
...
static const struct vd55g_chip_info vd55g1_chip_info = {
	...
	.patch_length = sizeof(vd55g1_patch_array),
	.patch_array = vd55g1_patch_array,
	.patch_revision_major = VD55G1_FWPATCH_REVISION_MAJOR,
	.patch_revision_minor = VD55G1_FWPATCH_REVISION_MINOR,
...
static int vd55g_patch(struct vd55g *sensor)
{
	u64 patch;
	int ret = 0;

	if (sensor->info->needs_patch) {
		vd55g_write_array(sensor, VD55G_REG_FWPATCH_START_ADDR,
				   sensor->info->patch_length,
				   sensor->info->patch_array, &ret);
		...
		vd55g_cci_read(sensor, REG_FWPATCH_REVISION, &patch, &ret);
		if (patch != (sensor->info->patch_revision_major << 8) +
		    sensor->info->patch_revision_minor) {
			dev_err(sensor->dev, "Bad patch version expected %d.%d got %d.%d\n",
				sensor->info->patch_revision_major,
				sensor->info->patch_revision_minor,
