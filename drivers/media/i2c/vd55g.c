...
#define VD55G0_REG_FWPATCH_REVISION			CCI_REG16_LE(0x0022)
...
#define VD55G0_REG_SYSTEM_FSM		CCI_REG8(0x002c)
...
#define VD55G0_REG_ISL_ENABLE				CCI_REG8(0x0329)
#define VD55G1_REG_ISL_ENABLE				CCI_REG8(0x0326)
#define VD55G_REG_OIF_IMG_CTRL				CCI_REG8(0x030f)
#define VD55G_REG_MIPI_DATA_RATE			CCI_REG32_LE(0x0224)
#define VD55G0_REG_PATGEN_CTRL				CCI_REG16_LE(0x0400)
...
#define VD55G0_REG_MANUAL_ANALOG_GAIN			CCI_REG8(0x044d)
#define VD55G1_REG_MANUAL_COARSE_EXPOSURE		CCI_REG16_LE(0x0502)
#define VD55G0_REG_MANUAL_COARSE_EXPOSURE		CCI_REG16_LE(0x044e)
#define VD55G1_REG_MANUAL_DIGITAL_GAIN			CCI_REG16_LE(0x0504)
#define VD55G0_REG_MANUAL_DIGITAL_GAIN			CCI_REG16_LE(0x0450)
#define VD55G1_REG_APPLIED_COARSE_EXPOSURE		CCI_REG16_LE(0x00e8)
#define VD55G0_REG_APPLIED_COARSE_EXPOSURE		CCI_REG16_LE(0x0064)
#define VD55G1_REG_APPLIED_ANALOG_GAIN			CCI_REG16_LE(0x00ea)
#define VD55G0_REG_APPLIED_ANALOG_GAIN			CCI_REG16_LE(0x0066)
#define VD55G1_REG_APPLIED_DIGITAL_GAIN			CCI_REG16_LE(0x00ec)
#define VD55G0_REG_APPLIED_DIGITAL_GAIN			CCI_REG16_LE(0x0068)
#define VD55G1_REG_AE_FORCE_COLDSTART			CCI_REG8(0x0308)
#define VD55G0_REG_AE_FORCE_COLDSTART			CCI_REG8(0x042c)
#define VD55G1_REG_AE_COLDSTART_EXP_TIME		CCI_REG32_LE(0x0374)
#define VD55G1_REG_AE_COLDSTART_COARSE_EXP		CCI_REG32_LE(0x042e)
#define VD55G0_REG_READOUT_CTRL				CCI_REG8(0x047a)
#define VD55G1_REG_READOUT_CTRL				CCI_REG8(0x057e)
#define VD55G_READOUT_CTRL_BIN_MODE_NORMAL		0
#define VD55G_READOUT_CTRL_BIN_MODE_DIGITAL_X2		1
#define VD55G_READOUT_CTRL_BIN_MODE_DIGITAL_X4		2
#define VD55G0_REG_DUSTER_CTRL				CCI_REG8(0x0316)
...
#define VD55G0_REG_AE_TARGET_PERCENTAGE			CCI_REG8(0x0440)
#define VD55G1_REG_NEXT_CTX				CCI_REG16_LE(0x03e4)
// #define VD55G0_REG_NEXT_CTX				CCI_REG16_LE(0x0478)
...
// #define VD55G0_REG_EXPOSURE_MAX_COARSE			CCI_REG16_LE(0x096c)
...
#define VD55G1_REG_MAX_EXPOSURE_LINES CCI_REG16_LE(0x096c)
...
static const char * const vd55g_supply_name[] = {
	"dvdd", /* VCORE (Digital core power supply) */
	"dovdd", /* VDDIO (Digital IO power supply) */
	"avdd", /* VANA (Analog power supply) */
};
...
static const int vd55g0_reg_map[REG_MAX_INDEX] = {
	[REG_SYSTEM_FSM] = VD55G0_REG_SYSTEM_FSM,
	[REG_ISL_ENABLE] = VD55G0_REG_ISL_ENABLE,
	[REG_FWPATCH_REVISION] = VD55G0_REG_FWPATCH_REVISION,
	[REG_PATGEN_CTRL] = VD55G0_REG_PATGEN_CTRL,
	[REG_READOUT_CTRL] = VD55G0_REG_READOUT_CTRL,
	[REG_DUSTER_CTRL] = VD55G0_REG_DUSTER_CTRL,
	[REG_AE_TARGET_PERCENTAGE] = VD55G0_REG_AE_TARGET_PERCENTAGE,
...
static const int vd55g1_reg_map[REG_MAX_INDEX] = {
	[REG_SYSTEM_FSM] = VD55G1_REG_SYSTEM_FSM,
	[REG_ISL_ENABLE] = VD55G1_REG_ISL_ENABLE,
	[REG_FWPATCH_REVISION] = VD55G1_REG_FWPATCH_REVISION,
	[REG_PATGEN_CTRL] = VD55G1_REG_PATGEN_CTRL,
	[REG_READOUT_CTRL] = VD55G1_REG_READOUT_CTRL,
	[REG_DUSTER_CTRL] = VD55G1_REG_DUSTER_CTRL,
	[REG_AE_TARGET_PERCENTAGE] = VD55G1_REG_AE_TARGET_PERCENTAGE,
	...
	[REG_CTX_REPEAT_COUNT_CTX0] = VD55G1_REG_CTX_REPEAT_COUNT_CTX0,
	[REG_CTX_REPEAT_COUNT_CTX1] = VD55G1_REG_CTX_REPEAT_COUNT_CTX1,
...
static inline int vd55g_get_ctx_addr(struct vd55g *sensor, enum vd55g_reg_id reg, u8 ctx)
{
...
static int vd55g_cci_read(struct vd55g *sensor, enum vd55g_reg_id reg, u64 *val, int *err)
{
	u32 physical_addr;
	...
	if (!physical_addr) {
		dev_err(sensor->dev, "Register %d not supported on this variant\n", reg);
		if (err)
			*err = -EOPNOTSUPP;
		return -EOPNOTSUPP;
	}
...
static int vd55g_cci_write(struct vd55g *sensor, enum vd55g_reg_id reg, u32 val, int *err)
{
	u32 physical_addr;
...
static int vd55g_poll_reg(struct vd55g *sensor, u32 reg, u8 poll_val,
			   int *err)
{
	unsigned int val = 0;
	int ret;
...
static int vd55g_update_expo_cluster(struct vd55g *sensor, bool is_auto)
{
	...
	if (sensor->info->hdr) {
		if (sensor->hdr_ctrl->val == VD55G1_HDR_SUB &&
			...
			vd55g_write(sensor, VD55G_REG_EXP_MODE(sensor, 1), VD55G1_EXP_BYPASS,
				     &ret);
			if (ret)
				return ret;
		}
				      ...
				      unsigned int frame_length)
{
	...
	if (sensor->info->hdr)
		if (sensor->hdr_ctrl->val == VD55G1_HDR_SUB)
			vd55g_write(sensor, VD55G_REG_FRAME_LENGTH(sensor, 1), frame_length,
				     &ret);
	vd55g_write(sensor, VD55G_REG_FRAME_LENGTH(sensor, 0), frame_length, &ret);
...
static int vd55g_update_hdr_mode(struct vd55g *sensor)
{
	...
	case VD55G1_HDR_SUB:
		...
		vd55g_write(sensor, VD55G_REG_EXPOSURE_INSTANCE(sensor, 0), 0, &ret);
		...
		vd55g_write(sensor, VD55G_REG_MASK_FRAME_CTRL(sensor, 1),
			     VD55G1_MASK_FRAME_CTRL_OUTPUT, &ret);
		vd55g_write(sensor, VD55G_REG_EXPOSURE_INSTANCE(sensor, 1), 1, &ret);
		break;
	default:
		ret = -EINVAL;
	}
			       ...
			       struct v4l2_rect *crop)
{
	...
	if (sensor->info->hdr) {
		vd55g_write(sensor, VD55G_REG_X_START(sensor, 1), crop->left, &ret);
		vd55g_write(sensor, VD55G_REG_X_WIDTH_OR_END(sensor, 1), x_val, &ret);
		vd55g_write(sensor, VD55G_REG_Y_START(sensor, 1), crop->top, &ret);
		vd55g_write(sensor, VD55G_REG_Y_HEIGHT_OR_END(sensor, 1), y_val, &ret);
	}
		...
		if (gpio_val == VD55G_GPIO_MODE_STROBE &&
			...
			if (sensor->info->hdr) {
				if (sensor->hdr_ctrl->val == VD55G1_HDR_SUB) {
					...
					vd55g_write(sensor,
						     VD55G_REG_GPIO_CTRL(sensor, 1, io),
						     gpio_val, &ret);
				}
			}
		}
				 ...
				 u64 streams_mask)
{
	...
	/* Configure output */
	vd55g_write(sensor, VD55G_REG_MIPI_DATA_RATE,
...
static int vd55g_load_and_apply_patch(struct vd55g *sensor, const char *fw_name)
{
	...
	ret = request_firmware(&fw, fw_name, sensor->dev);
	if (ret) {
		dev_err(sensor->dev, "Failed to load firmware patch file %s: %d\n", fw_name, ret);
		return ret;
	}

	if (fw->size < sizeof(*hdr)) {
		dev_err(sensor->dev, "Patch file is too small to contain a header\n");
		return -EINVAL;
	}
	...
	if (payload_len > fw->size) {
		dev_err(sensor->dev, "Patch payload length mismatch in header\n");
		return -EINVAL;
	}

	u32 target_reg = VD55G_REG_FWPATCH_START_ADDR;

	vd55g_write_array(sensor, target_reg, fw->size, fw->data, &ret);
	vd55g_write(sensor, VD55G_REG_BOOT,
		     VD55G_BOOT_PATCH, &ret);
	...
	ret = vd55g_cci_read(sensor, REG_FWPATCH_REVISION, &hardware_version, &ret);
	if (ret)
		return ret;

	u64 expected_version = (hdr->major << 8) | hdr->minor;
	if ((hardware_version & 0xFFFF) != expected_version) {
			      ...
			      struct v4l2_subdev_format *sd_fmt)
{
	...
	*v4l2_subdev_state_get_crop(sd_state, sd_fmt->pad) = pad_crop;
	if (sd_fmt->which == V4L2_SUBDEV_FORMAT_ACTIVE)
		return vd55g_new_format_change_controls(sensor,
							 &sd_fmt->format,
							 &pad_crop);
...
disable_bulk:
	regulator_bulk_disable(ARRAY_SIZE(vd55g_supply_name),
			       sensor->supplies);
...
static int vd55g_parse_dt_gpios_array(struct vd55g *sensor,
