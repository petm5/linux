...
#define VD55G1_LINE_LENGTH_MIN 1200
...
struct vd55g1_chip_info {
	...
	u16 num_modes;
	const int *reg_map;
	const struct vd55g1_fw_revision_map *fw_maps;
	u16 num_fw_maps;
...
static const struct vd55g1_fw_revision_map vd55g0_fw_maps[] = {
	{
		.revision = 0x1111,
		.needs_patch = true,
...
static const struct vd55g1_fw_revision_map vd55g1_fw_maps[] = {
	{
		.revision = 0x2020,
		.needs_patch = true,
		.fw_name = "vd55g1.bin",
...
static const struct vd55g1_fw_revision_map vd65g4_fw_maps[] = {
	{ .revision = 0x3030, .needs_patch = false },
};

static const struct vd55g1_chip_info vd55g0_chip_info = {
	.id = 0x53354730,
	.name = "vd55g0",
...
static int vd55g1_read_ctx_offset(struct vd55g1 *sensor, enum vd55g1_reg_id reg, u8 ctx, u8 offset, u64 *val,
			  int *err)
{
	u32 addr;
	int ret;
...
static int vd55g1_write_ctx_offset(struct vd55g1 *sensor, enum vd55g1_reg_id reg, u8 ctx, u8 offset, u32 val,
			   int *err)
{
	u32 addr;
	int ret;
...
static int vd55g1_write_array(struct vd55g1 *sensor, enum vd55g1_reg_id reg, unsigned int len,
...
static int vd55g1_update_expo_cluster(struct vd55g1 *sensor, bool is_auto)
{
	...
	if (!is_auto && sensor->dgain_ctrl->is_new)
		vd55g1_write(sensor, REG_MANUAL_DIGITAL_GAIN,
...
static int vd55g1_apply_cold_start(struct vd55g1 *sensor, struct v4l2_rect *crop)
{
	int ret;
...
static int vd55g1_load_and_apply_patch(struct vd55g1 *sensor,
				      const struct vd55g1_fw_revision_map *fw_info)
{
	...
	vd55g1_write_array(sensor, REG_FWPATCH_START_ADDR, fw->size,
			  fw->data, &ret);
	vd55g1_write(sensor, REG_BOOT, VD55G1_BOOT_PATCH, &ret);
	...
	ret = vd55g1_read(sensor, REG_FWPATCH_REVISION, &hardware_version,
			     &ret);
	if (ret)
		return ret;
...
static int vd55g1_get_firmware_name(struct vd55g1 *sensor, u64 revision,
				   const struct vd55g1_fw_revision_map **fw_info)
{
	for (int i = 0; i < sensor->info->num_fw_maps; i++) {
		const struct vd55g1_fw_revision_map *map = &sensor->info->fw_maps[i];
...
static int vd55g1_boot(struct vd55g1 *sensor)
{
	...
	ret = vd55g1_read(sensor, REG_REVISION, &rev, NULL);
	if (ret)
		return ret;

	ret = vd55g1_get_firmware_name(sensor, rev, &fw_info);
	if (ret == -ENOENT) {
		dev_err(sensor->dev,
	...
	if (fw_info->needs_patch)
		vd55g1_load_and_apply_patch(sensor, fw_info);
			     ...
			     struct v4l2_subdev_format *sd_fmt)
{
	...
	*v4l2_subdev_state_get_crop(sd_state, sd_fmt->pad) = pad_crop;
	if (sd_fmt->which == V4L2_SUBDEV_FORMAT_ACTIVE)
		return vd55g1_new_format_change_controls(sensor, &sd_fmt->format,
							&pad_crop);
			    ...
			    struct v4l2_subdev_state *sd_state)
{
	...
	vd55g1_update_pad_fmt(sensor, &sensor->info->modes[VD55G1_MODE_DEF],
...
static int vd55g1_detect(struct vd55g1 *sensor)
{
	...
	ret = vd55g1_read(sensor, REG_MODEL_ID, &id, NULL);
	if (ret)
		return ret;
...
disable_bulk:
	regulator_bulk_disable(ARRAY_SIZE(vd55g1_supply_name), sensor->supplies);
...
static int vd55g1_parse_dt_gpios_array(struct vd55g1 *sensor, char *prop_name,
				      u32 *array, int *nb)
{
	unsigned int i;
	int ret;
