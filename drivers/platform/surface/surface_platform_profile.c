...
static ssize_t ssam_fan_reset(struct ssam_device *sdev)
{
		int status;

		status = ssam_fan_profile_set(sdev, SSAM_FAN_PROFILE_NORMAL);
		if (status)
			return status;

		status = ssam_fan_state_set(sdev, SSAM_FAN_STATE_NORMAL);
		if (status)
			return status;

		status = ssam_fan_tmp_offset_set(sdev, 0);
		if (status)
			return status;

		status = ssam_fan_base_rpm_set(sdev, 0);
